...
func (c *Config) SetVolumes(volumes []machine_config.Volume) {
...
func GetPlatformVersion(ctx context.Context, path string) (string, error) {
	client := client.FromContext(ctx)
	slimConfig := &SlimConfig{}
	...
	_, err = toml.NewDecoder(file).Decode(&slimConfig)
